...
Scope: current promoted production path, Forest -> Cave -> Sand -> Swamp -> Snow. Values come from the shipped runtime in `scripts/game.js`, with normal enemy density and the deterministic feel/progression smoke assumptions.
...
- The early primary-element reward path biases a one-element run toward at least one compatible fusion setup in Forest, Cave, Sand, Swamp, and Snow.
...
- Reload checks preserve promoted unlock state through Ocean Land visibility.

## Coverage Notes

- `smoke:feel` protects opener roster width, opening spawn interval, opening enemy cap, first-three-wave variety, catalyst milestones, pickup tuning, boss tuning multipliers, and early fusion setup for all five worlds.
- `smoke:progression` protects save unlocks, alchemy recipe persistence, character rewards, essence rewards, and Snow reload stats through the current promoted path.
...
- Ocean is unlocked by the promoted path but is not promoted yet. It still needs Ocean tuning, deterministic coverage, Sea Kings hardening, and a stable named live gate before release claims include it.
