import { ENEMY_TYPES, getEnemyStats } from '../../data/EnemyConfig.js';
...
export class EnemyManager {
    ...
    setupEventListeners() {
        ...
        this.scene.events.on('spawnEnemy', (data) => {
    ...
    spawnEnemy(type, x, y, variant = null) {
        ...
        switch (type) {
            case 'golem':
                enemy = this.createGolem(x, y, variant || 'orange', stats);
                break;
            case 'slime':
    ...
    createBasicEnemy(x, y, type, stats) {
        const config = ENEMY_TYPES[type];
        if (!config) return null;
        
        const enemy = this.scene.physics.add.sprite(x, y, config.sprite);
    ...
    createGolem(x, y, color, stats) {
        const enemy = this.scene.physics.add.sprite(x, y, `golem-${color}-walk`);
    ...
    createSlime(x, y, stats) {
        const enemy = this.scene.physics.add.sprite(x, y, 'slime-idle-0');
    ...
    createSummoner(x, y, stats) {
        const enemy = this.scene.physics.add.sprite(x, y, 'summoner-idle');
    ...
    createSoul(x, y, stats) {
        const enemy = this.scene.physics.add.sprite(x, y, 'soul-idle-0');
    ...
    createDarkEye(x, y, stats) {
        const enemy = this.scene.physics.add.sprite(x, y, 'darkeye-walk-1');
    ...
    update(time, delta) {
        ...
        this.enemies.children.entries.forEach(enemy => {
            ...
            this.updateEnemyFacing(enemy, wizard);
            
            // Special enemy behaviors
            this.updateSpecialBehaviors(enemy, time);
        });
    ...
    updateEnemyAI(enemy, target, time) {
        ...
        if (enemy.enemyType === 'soul' && distance < enemy.attackRange) {
            // Stop and attack
            enemy.setVelocity(0, 0);
    ...
    updateEnemyFacing(enemy, target) {
    ...
    updateSpecialBehaviors(enemy, time) {
        // Summoner behavior
    ...
    handleSoulAttack(soul, target, time) {
        if (time - soul.lastAttackTime > soul.attackCooldown) {
            // Create projectile
            const angle = Math.atan2(target.y - soul.y, target.x - soul.x);
            const projectile = this.scene.physics.add.sprite(soul.x, soul.y, 'soul-projectile');
    ...
    getEnemyCount() {
        return this.enemies.children.entries.filter(e => e.active).length;
    }
