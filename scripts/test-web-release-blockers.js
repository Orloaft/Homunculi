...
assert(gameSource.includes("CASTLE CLEARED • WEB RELEASE COMPLETE"), 'Castle ending has no compact completion marker');
assert(gameSource.includes("showRunRewardReadback(delay) {\n        // The Castle clear is the web-release ending. Its compact ending layout\n        // needs the results and replay action, not the full mid-screen readback.\n        if (this.releaseComplete) return delay;"), 'Castle ending still renders the overlapping run-readback panel');
assert(gameSource.includes("const endingStatLayout = this.releaseComplete ? { time: 230, kills: 265, items: 300, score: 335, essence: 374 } :"), 'Castle ending does not reserve a separate results lane');
assert(gameSource.includes("const endingActionLayout = this.releaseComplete ? { primary: 435, menu: 470, fontSize: '20px' } :"), 'Castle ending does not reserve a separate replay-action lane');
